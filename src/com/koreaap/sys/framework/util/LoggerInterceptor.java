...
public class LoggerInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(cookies != null) {
			...
			String userIp = (String) cookieInfo.get("userIp") != null ? (String) cookieInfo.get("userIp") : "" ;
	...
	private boolean isLogginUrl(String requestUrl) {
		if(requestUrl.startsWith("/resources")) {
			return false;
		} else if(requestUrl.startsWith("/not")) {
			return false;
		} else {
			return true;
		}
	...
	public void postHandle(HttpServletRequest request,
		...
		// resources 경로를 제외한 모든 경로 접속 시 쿠키에 정보를 담는다.
		if(isLogginUrl(request.getRequestURI())) {
			Cookie cookie = new Cookie("userId", UserInfoHolder.getUserId());
			response.addCookie(cookie);
			cookie = new Cookie("userIp", request.getRemoteAddr());
			response.addCookie(cookie);
